...
class TestA(TestCase):
    ...
    def test_univariate_quadrature_rules(self):
        ...
        np.testing.assert_array_almost_equal(float(pts[0, 0]), -1.0, decimal=5, err_msg='Problem!')
    ...
    def test_univariate_quadrature_rules(self):
        ...
        poly = Poly([param], basis, method='numerical-integration')
        pts = poly.get_points()
        np.testing.assert_array_almost_equal(float(pts[0, 0]), -15.5, decimal=5, err_msg='Problem!')
        np.testing.assert_array_almost_equal(float(pts[20, 0]), 32.2, decimal=5, err_msg='Problem!')
        param = Parameter(distribution='uniform', lower=32.2, upper=68123.1, order=10, endpoints='lower')
        basis = Basis('univariate')
        poly = Poly([param], basis, method='numerical-integration')
        pts = poly.get_points()
        np.testing.assert_array_almost_equal(float(pts[0, 0]), 32.2, decimal=5, err_msg='Problem!')
        param = Parameter(distribution='uniform', lower=-135.32, upper=68123.122, order=10, endpoints='upper')
        basis = Basis('univariate')
        poly = Poly([param], basis, method='numerical-integration')
        pts = poly.get_points()
        np.testing.assert_array_almost_equal(float(pts[10, 0]), 68123.122, decimal=5, err_msg='Problem!')
